        ...
        <div class="card-body">
            <div class="row">
                ...
                <div class="col-7 fc-rtl">
                    <a href="{{ route('adm.donatur.shorten-link.index', $donatur->id) }}" class="btn btn-outline-dark"> kembali</a>
                </div>
            </div>
            ...
            <form action="{{ route('adm.donatur.shorten-link.store') }}" method="post"
                accept-charset="utf-8" class="row gy-3">
                @csrf
                <input type="text" name="donatur_id" class="d-none" value="{{ $donatur->id }}"/>
                {{-- url tujuan --}}
                <div class="col-12">
                    <label class="form-label fw-semibold">Tampilan URL/Link Awal</label>
                    <div class="input-group input-group-sm">
                        <span class="input-group-text">
                            https://bantubersama.com/<span id="program-slug"></span><span id="nominal"></span><span id="payment_type_url"></span>?name={{ $donatur->name }}&telp={{ $donatur->telp }}</span>
                ...
                <div class="col-12">
                    ...
                    <div class="input-group input-group-sm">
                        ...
                        <input class="form-control form-control-sm" id="rupiah" name="amount"
                            placeholder="0" type="text" value="" required />
                    </div>
                    @error('amount')
                ...
                <div class="col-12">
                    ...
                    <select name="payment_type" id="payment_type" class="form-control form-control-sm" required>
                        @forelse($payment_types as $payment_type)
                            <option value="{{ $payment_type->key }}" {{ old('payment_type') == $payment_type->key ? 'selected' : '' }}>{{ $payment_type->name }}</option>
                        @empty
                            <span>
