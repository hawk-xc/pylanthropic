@extends('layouts.admin', [
    'second_title' => 'Edit CRM Pipeline',
    'header_title' => 'Edit CRM Pipeline',
...
@section('content')
    <div class="row">
        <div class="col-12">
            ...
            <form id="pipeline-form" action="{{ route('adm.crm-pipeline.update', $pipeline->id) }}" method="post"
                ...
                <div class="card border-0 shadow-sm">
                    <div class="card-header bg-light">
                        <h5 class="card-title mb-0">Formulir Edit Pipeline</h5>
                    </div>
                    <div class="card-body">
                        <div class="row">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" id="description" name="description" rows="4"
                                placeholder="Jelaskan tentang pipeline ini...">{{ old('description', $pipeline->description) }}</textarea>
                            @error('description')
                                <div class="text-danger small mt-1"><i class="ri-error-warning-line"></i>
                                    {{ $message }}</div>
                            @enderror
                        ...
                        <div class="mb-3 ml-3">
                            <div class="form-check form-switch">
                                ...
                                <input type="hidden" name="is_active_hidden" id="status_is_active_hidden"
                                    value="{{ $pipeline->is_active ? 1 : 0 }}">
                            </div>
                        </div>
                    </div>
                    <div class="card-footer text-end bg-light flex flex-row gap-3">
                        ...
                        <button type="submit" class="btn btn-primary">
                            <i class="ri-save-line"></i> Update Pipeline
                        </button>
                    </div>
                </div>
            </form>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $('#pipeline-form').on('reset', function() {
                ...
                $('#status_is_active').prop('checked', {{ $pipeline->is_active ? 'true' : 'false' }})
                    .trigger('change');
            });
        });
    </script>
