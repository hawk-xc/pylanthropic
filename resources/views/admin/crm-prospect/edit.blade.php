            ...
            <div class="row">
                    ...
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb mb-0 pb-0 pl-0">
                            ...
                            <li class="breadcrumb-item active" aria-current="page"><a
                                    href="{{ route('adm.crm-leads.index') }}">Leads CRM</a></li>
                ...
                <div class="col-7 fc-rtl">
                    <a class="btn btn-outline-primary"
                        href={{ route('adm.crm-pipeline.index', ['type' => request()->query('type')]) }}>Kembali</a>
                </div>
            </div>
            <div class="divider"></div>
            <form action="{{ route('adm.crm-prospect.update', $crm_prospect->id) }}?leads={{ request()->query('leads') }}" method="post" accept-charset="utf-8"
                class="row gy-3">
                @csrf
                @method('PUT')
                <div class="col-12">
                    <label class="form-label">Nama Prospect</label>
                    <input type="text" class="form-control" name="name" id="name"
                        placeholder="Masukkan Nama Prospect" value="{{ old('name', $crm_prospect->name) }}" required>
                    @error('name')
                ...
                <div class="col-12">
                    ...
                    <select class="form-control" name="pipeline" id="pipeline" required>
                        <option value="">Pilih Pipeline</option>
                        @forelse ($pipelines as $pipeline)
                            <option value="{{ $pipeline->id }}"
                                {{ $pipeline->id == $crm_prospect->crm_pipeline_id ? 'selected' : '' }}>
                                {{ $pipeline->name }}
                            </option>
                        @empty
                ...
                <div class="col-12">
                    ...
                    <select class="form-control form-control-sm" name="donatur" id="donatur-select2" required>
                        @if ($crm_prospect->donatur_id)
                            <option value="{{ $crm_prospect->donatur_id }}" selected>
                ...
                <div class="col-12">
                    <label class="form-label required">Pilih PIC</label>
                    <select id="users-select2" name="assign_to" class="form-control">
                        @if ($crm_prospect->assign_to)
                            <option value="{{ $crm_prospect->assign_to }}" selected>
                                {{ $crm_prospect->crm_prospect_pic->name }}
                            </option>
                        @endif
                    </select>
                    @error('assign_to')
                ...
                <div class="col-12">
                    ...
                    <textarea class="form-control" id="description" name="description" rows="3" placeholder="Masukkan deskripsi">{{ old('description', $crm_prospect->description) }}</textarea>
                </div>

                <div class="col-12">
                    <label class="form-label">Nominal Propect</label>
                    <div class="input-group input-group-sm">
                        <span class="input-group-text">Rp. </span>
                        <input type="text" class="form-control" name="nominal" id="rupiah" placeholder="100.000.000"
                            value="{{ old('nominal', number_format($crm_prospect->nominal, 0, ',', '.')) }}" required>
                        @error('nominal')
                            <div class="text-danger small mt-1"><i class="ri-error-warning-line"></i> {{ $message }}
                            </div>
                        @enderror
                ...
                <div class="col-12">
                    <div class="form-check form-switch mt-2 ml-3">
                        <input class="form-check-input" type="checkbox" name="is_potential" id="is_potential" value="1"
                            {{ old('is_potential' === 1 ? 'checked' : '', 'checked') }}>
                        <label class="form-check-label" for="is_potential">Status Potensial (<span
                                id="_status">Ya</span>)</label>
                        <input type="hidden" name="is_potential_hidden" id="is_potential_hidden" value="1">
                    </div>
                </div>
...
@section('js_inline')
    <script type="text/javascript">
        $(document).ready(function() {
            const prospectDonaturId = @json($crm_prospect->donatur_id ?? null);
            const prospectAssignToId = @json($crm_prospect->assign_to ?? null);
            const donaturName = @json($crm_prospect->donatur->name ?? '');
            const assignToName = @json($crm_prospect->assignToUser->name ?? '');

            // Inisialisasi Select2 Donatur
            ...
            if (prospectDonaturId && donaturName) {
                var donaturOption = new Option(donaturName, prospectDonaturId, true, true);
                $("#donatur-select2").append(donaturOption).trigger('change');
            }

            if (prospectAssignToId && assignToName) {
                var assignToOption = new Option(assignToName, prospectAssignToId, true, true);
                $("#users-select2").append(assignToOption).trigger('change');
            }
